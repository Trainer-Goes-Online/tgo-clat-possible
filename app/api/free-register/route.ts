...
// FREE funnel registration. No payment / no Razorpay. On submit we fire the Meta
// CAPI events (CompleteRegistration + a custom event, both env-driven) and POST
// the same 23-field enriched payload to the funnel's Pabbly webhook, then the
// client redirects to /confirmation.
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Env-driven so each free deployment (new pixel vs old pixel) can tune them.
    const standardEvent =
