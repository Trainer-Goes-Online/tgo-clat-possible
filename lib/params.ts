...
// across LP -> /checkout -> /confirmation so the full attribution chain reaches
// the Pabbly webhook + Meta CAPI on a successful purchase.
...
function writeStoredParams(params: ParamMap) {
  ...
  document.cookie = `${PARAMS_COOKIE_NAME}=${value}; Path=/; Max-Age=${COOKIE_TTL_SECONDS}; SameSite=Lax; Secure`;
}
