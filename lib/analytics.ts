...
function writeMamCookie(matching: Record<string, string>) {
  ...
  document.cookie = `${MAM_COOKIE_NAME}=${value}; Path=/; Max-Age=${COOKIE_TTL_SECONDS}; SameSite=Lax; Secure`;
}
 ...
 * Call sites: form-fill effect, payment-success handler.
 */
...
/** Re-fire MAM from the persisted cookie (safety net on the thank-you page). */
